...
Скопируйте `.env.example` в `.env` в корне проекта (PowerShell: `Copy-Item .env.example .env`). В шаблоне указаны стандартные пути к eSpeak NG для Windows. Локальный `.env` не загружается в Git. Если eSpeak NG установлен в другую папку, измените `PHONEMIZER_ESPEAK_LIBRARY` и `PHONEMIZER_ESPEAK_DATA_PATH`.
...
- `english_list.xlsx` или `spanish_list.xlsx` — список для изучения с четырьмя столбцами:
  слово с лицевой стороны карточки, транскрипция, перевод и словоформы с оборота.
  Для испанского оба перевода находятся в одной ячейке на отдельных строках.
  Список создаётся вместе с карточками (`cards.enabled: true`), использует тот же
  отбор и исключение известных слов; отдельная команда `cards` также обновляет список.

В английском XLSX пустые столбцы «Род», «Группа спряжения», «Регулярность»
и «Перевод на английский» не выводятся; если данные есть, столбец сохраняется.
CSV сохраняет полный набор столбцов. «Количество контекстных блоков» расположено
между специфичностью и транскрипцией. Ширина выбранных столбцов английского Excel
задана в пересчёте из пикселей (38, 120 и 180 при стандартном масштабе).
...
Отбор выполняется по группам одинаковых лемм: частоты разных частей речи суммируются.
Например, `watch/VERB` и `watch/NOUN` проходят отбор вместе, а `venir/VERB`
и `vino/NOUN` остаются разными группами. Покрытие считает каждую группу один раз,
включая группы, добавленные по специфичности. Фоновая частота группы — сумма
частот уникальных наблюдаемых словоформ, без повторного учёта общих форм.

В таблице лемм строки разных частей речи сохраняются и стоят рядом с общим рангом.
«Суммарное число вхождений» относится к отдельной части речи, следующий столбец
«Всего вхождений леммы» — ко всем частям речи этой леммы. Доля от всех слов,
кумулятивное покрытие, относительная частота в книге, частота Wordfreq и специфичность
рассчитываются по всей лемме, как и отбор для перевода и карточек. Покрытие увеличивается
один раз на лемму; её строки показывают одинаковое значение. Общие показатели нельзя
суммировать по повторяющимся строкам. «Доля от всех слов» и «Относительная частота в книге»
показывают одну величину: общее число вхождений леммы / число учтённых словоупотреблений.
На группу создаётся одна карточка: переводы объединяются через запятую,
словоформы объединяются без повторений. Перевод каждой части речи использует её собственные контексты.
