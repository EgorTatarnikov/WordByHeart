...
```
В шаблоне указаны стандартные пути к eSpeak NG для Windows. Локальный `.env` не загружается в Git. Если eSpeak NG установлен в другую папку, измените `PHONEMIZER_ESPEAK_LIBRARY` и `PHONEMIZER_ESPEAK_DATA_PATH`.
...
- Cписок cлов для изучения (тех же что и в карточках);
